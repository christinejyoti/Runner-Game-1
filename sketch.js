...
function preload() {
  ironMan_Image = loadImage("iorn man.gif");
...
function setup() {
  ...
  ironMan = createSprite(200, 360, 10, 10);
...
function draw() {
  ...
  if (gameState === START) {
    ...
    camera.position.y = ironMan.y - 167
    ironMan.velocityY = -10
    side1.y = ironMan.y - 200
    side2.y = ironMan.y - 200
    ...
    ironMan.collide(side1);
    ironMan.collide(side2);
    ironMan.setCollider("rectangle", 0, 0, 170, 250);
    ...
    if (ironMan.isTouching(missileGroup) || keyDown("k") /*k=kill*/) {
  ...
  if (gameState === PLAY || gameState === END) {
    ...
    fill(1);
    textSize(20);
    text("score:" + score, 150, ironMan.y - 330)
  }
  if (gameState === END) {
    noStroke();
    fill(1);
...
function spawnMissile() {
  missile = createSprite(0, ironMan.y - 400, 10, 10);
...
function spawnCoins() {
  coin = createSprite(0, ironMan.y - 400, 10, 10);
...
function spawnClouds() {
  if (frameCount % 15 === 0) {
    cloud = createSprite(600, ironMan.y - 400, 40, 10);
